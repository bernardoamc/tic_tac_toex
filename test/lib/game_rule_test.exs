defmodule TicTacToex.GameRuleTest do
  ...
  setup do
    {:ok, game_state} =
      GameRule.init
      |> GameRule.add_player("setup_player")

    {:ok, %{game: game_state}}
  end

  test "add_player/2 adds a player and update the game status", %{game: game_state} do
    {:ok, new_state} = GameRule.add_player(game_state, "new_player")
  ...
  test "add_player/2 gives and error without empty spots", %{game: game_state} do
    {:ok, new_state} = GameRule.add_player(game_state, "new_player")
    {response, _}  = GameRule.add_player(new_state, "full_game")
  ...
  test "play_at/2 when a player has won", %{game: game_state} do
    ...
    x_won_state = GameRule.play_at(x_to_win_state, "setup_player", 2)
  ...
  test "play_at/2 when a draw happens", %{game: game_state} do
    ...
    draw_state = GameRule.play_at(to_draw_state, "setup_player", 8)
  ...
  test "play_at/2 when the game is not over", %{game: game_state} do
    {:ok, running_state} = GameRule.add_player(game_state, "new_player")

    assert running_state[:status] == :running
    assert running_state[:turn] == :x

    new_state = GameRule.play_at(running_state, "setup_player", 1)
